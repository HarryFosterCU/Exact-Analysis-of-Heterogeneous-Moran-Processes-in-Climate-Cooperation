...
df_imispection = pd.read_csv(root_path / "assets/data/raw/binomial_population_imispection/main.csv")
df_introspection = pd.read_csv(root_path / "assets/data/raw/binomial_population_introspection/main.csv")
...
df_imispection["alpha_range"] = data_group.transform("max") - data_group.transform("min")
...
df_introspection["alpha_range"] = data_group.transform("max") - data_group.transform("min")
df_introspection["std_alpha"] = data_group.transform("std")
df_introspection = df_introspection.groupby("UID").first()


df_compare = (
    df_imispection.merge(
        df_introspection,
        on=["N", "r", "population", "beta", "alpha_range", "std_alpha"],
        how="inner",
        suffixes=("_in_imispection", "_in_introspection")
    )
)

df_compare = df_compare.drop(columns=["alpha_i_in_imispection", "i_in_imispection", "alpha_i_in_introspection", "i_in_introspection"])

conditions = [
    df_compare["p_C_in_imispection"] > df_compare["p_C_in_introspection"],
    df_compare["p_C_in_imispection"] < df_compare["p_C_in_introspection"],
    df_compare["p_C_in_imispection"] == df_compare["p_C_in_introspection"]
]

choices = ["introspective imitation", "introspection", "draw"]

df_compare["winner"] = np.select(conditions, choices, default="draw")
df_compare["winner_margin"] = np.abs(df_compare["p_C_in_imispection"] - df_compare["p_C_in_introspection"])
