...
df_moran = pd.read_csv(root_path / "assets/data/raw/binomial_population_moran/main.csv")
...
df_compare = (
    df_imispection.merge(
        df_moran,
        on=["mutant_alpha", "N", "r", "population", "epsilon", "alpha_range", "std_alpha"],
        how="inner",
        suffixes=("_in_imispection", "_in_moran")
    )
)

df_compare = df_compare.drop(columns=["alpha_i_in_imispection", "i_in_imispection", "alpha_i_in_moran", "i_in_moran"])

conditions = [
    df_compare["p_C_in_imispection"] > df_compare["p_C_in_moran"],
    df_compare["p_C_in_imispection"] < df_compare["p_C_in_moran"],
    df_compare["p_C_in_imispection"] == df_compare["p_C_in_moran"]
]

choices = ["introspective imitation", "moran", "draw"]

df_compare["winner"] = np.select(conditions, choices, default="draw")
df_compare["winner_margin"] = np.abs(df_compare["p_C_in_imispection"] - df_compare["p_C_in_moran"])
