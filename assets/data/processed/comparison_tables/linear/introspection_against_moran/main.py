...
dfm = pd.read_csv(root_path / "assets/data/raw/linear_population_moran/main.csv")
dfi = pd.read_csv(root_path / "assets/data/raw/linear_population_introspection/main.csv")

for N in dfm["N"].unique():
    if N in dfi.unique():
        print(N)
        df_moran = dfm[dfm["N"]==N]
        df_introspection = dfi[dfi["N"]==N]
        ...
        df_compare = df_compare.drop(columns=["alpha_i_in_moran", "i_in_moran", "alpha_i_in_introspection", "i_in_introspection"])
        ...
        df_compare["winner_margin"] = np.abs(df_compare["p_C_in_moran"] - df_compare["p_C_in_introspection"])

        df_compare.to_csv(here.parent / "main.csv", mode='a')