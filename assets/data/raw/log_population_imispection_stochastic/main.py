        ...
        for choice_intensity in choice_intensity_range:
            for scale in np.linspace(0.1, 10, 30):
                for repetitions in range(200):
                    ...
                    alphas = main.get_dirichlet_contribution_vector(N=N, alpha_rule=contribution_rules.dirichlet_log_alpha_rule, M=M, scale=scale)
                    for selection_intensity in np.linspace(0,(1/max(alphas)) * 0.99,30):
                        ...
                        for first_contribution in np.unique(alphas):
                            ...
                            for i, alpha in enumerate(alphas):
                                row = [id, alpha, i, first_contribution, N, r, choice_intensity, selection_intensity,p_C, "introspective imitation", "log", True]
                                data.append(row)
