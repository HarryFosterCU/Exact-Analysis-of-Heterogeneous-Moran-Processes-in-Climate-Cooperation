...
df = pd.read_csv(
    assets_path
    / "data/processed/comparison_tables/binomial/moran_against_imispection/main.csv"
)
...
for N, N_frame in df.groupby("N"):
    ...
    N_frame["p_C_difference"] = N_frame["p_C_in_imispection"] - N_frame["p_C_in_moran"]
    N_frame["r"] = N_frame["r"].round(3)

    sns.violinplot(data=N_frame, x="r", y="p_C_difference", ax=ax)
