...
def generate_stochastic_2_valency_cycle_matrix(n, prob_prior):
    I = np.eye(n)
    return prob_prior * (np.roll(I, -1, axis=1)) + (1 - prob_prior) * (np.roll(I, 1, axis=1))
...
def test_generate_stochastic_2_valency_cycle_matrix():
    ...
    np.testing.assert_array_equal(deterministic_matrix, generate_stochastic_2_valency_cycle_matrix(n=3, prob_prior=0))

    stochastic_matrix = np.array([[0, 0.3, 0, 0.7], [0.7, 0, 0.3, 0], [0, 0.7, 0, 0.3], [0.3, 0, 0.7, 0]])
    np.testing.assert_array_almost_equal(stochastic_matrix, generate_stochastic_2_valency_cycle_matrix(n=4, prob_prior=0.7))

def test_generate_full_uniform_matrix():
    """Tests that generate_full_uniform_matrix is correct"""

    uniform_matrix = np.array([[1/4,1/4,1/4, 1/4], [1/4,1/4,1/4, 1/4], [1/4,1/4,1/4, 1/4], [1/4,1/4,1/4, 1/4]])
    np.testing.assert_array_equal(uniform_matrix, generate_full_uniform_matrix(4))


@pytest.mark.parametrize("n",
    range(2, n_range, step_size_n))
...
@pytest.mark.parametrize("n, prob_prior", [
    (n, p)
    for n in range(2, n_range, step_size_n)
    for p in [1 / (i) for i in range(1, prob_range + 1, step_size_prob)]
])
def test_approximate_steady_state_for_stochastic_2_valency_cycle_matrix(n,prob_prior, benchmark):
