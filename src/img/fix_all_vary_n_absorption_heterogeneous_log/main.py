    ...
    for N in n_values:
        ...
        transition_matrix = main.generate_transition_matrix(
            ...
            contribution_vector=main.get_deterministic_contribution_vector(
                cr.log_contribution_rule, N=N, M=M
            ),
            **kwargs
        )
...
axes[8].axis("off")
fig.suptitle(
    r"Absorption probabilities for Full Cooperation in a logly Heterogeneous PGG with varying population size."
)
fig.text(
    0.5,
    -0.02,
...
axes[2].scatter(
    range(10),
    np.array([cr.log_contribution_rule(index=n, N=10, M=M) for n in range(10)]),
    label="",
)
