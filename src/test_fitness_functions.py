...
def test_heterogeneous_contribution_pgg_fitness_function_for_heterogeneous_numeric_contribution():
    ...
    actual_return = ff.heterogeneous_contribution_pgg_fitness_function(
        state=state, contribution_vector=heterogeneous_contributions, r=r, epsilon=epsilon
    )
...
def test_homogeneous_pgg_fitness_function_for_numeric_value():
    ...
    actual_return = ff.homogeneous_pgg_fitness_function(state=state, alpha=alpha, r=r, epsilon=epsilon)
...
def test_homogeneous_pgg_fitness_function_for_symbolic_values():
    ...
    expected_return = np.array([contributor_payment, contributor_payment, defector_payment])
...
def test_general_four_state_player_fitness_function_for_whole_state_space():
    ...
    four_state_space = main.get_state_space(N=2,k=2)

    f = sym.Function("f")

    expected_return_a = np.array([sym.Function(f"f_{1}")(sym.Symbol('a')),sym.Function(f"f_{2}")(sym.Symbol('a'))])

    expected_return_b = np.array([sym.Function(f"f_{1}")(sym.Symbol('b')),sym.Function(f"f_{2}")(sym.Symbol('b'))])

    expected_return_c = np.array([sym.Function(f"f_{1}")(sym.Symbol('c')),sym.Function(f"f_{2}")(sym.Symbol('c'))])

    expected_return_d = np.array([sym.Function(f"f_{1}")(sym.Symbol('d')),sym.Function(f"f_{2}")(sym.Symbol('d'))])

    expected_returns = [expected_return_a,expected_return_b,expected_return_c,expected_return_d]

    for i in range(4):
        np.testing.assert_array_equal(expected_returns[i],ff.general_four_state_fitness_function(four_state_space[i]))