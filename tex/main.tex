...
\section{The Model}
In this section, we examine the underlying framework governing a heterogeneous game. We begin by defining a population of individuals and the actions which they may take, and then we see different methods by which we can model the evolution of a population over time.
...
Now let $h(\textbf{a, b})$ denote the Hamming distance~\cite{Hamming_Distance} between two states \(\textbf{a}, \textbf{b} \in S\). 
The set of states \textbf{b} such that $h(\textbf{a, b}) = 1$ is defined as Neb(\textbf{a}), the ``Neighbourhood set of a state" \cite{Introspection Paper 2}.
Our system will be in the form of a Markov chain~\cite{Markov_Chain} which moves between states with a probability \(>0\) if and only if $h(\textbf{a, b}) \leq 1$. 
We call the index at which \textbf{a} and \textbf{b} differ the \textit{index of difference}, denoted \(I(\textbf{a,b})\) \cite{Introspection Paper 2}.
...
The Moran process \cite{Nowak 2006} is a population corresponding to the following algorithm:
\begin{enumerate}
...
\end{enumerate}
Let \((a_{-j})\) denote the state from the perspective of \({a_j}\), so that \(\textbf{a} = (a_j, a_{-j})\). Then, the entry \(T_\textbf{a,b}\) of a transition matrix \(T\) in which each row and column correspond to a state in \(S\) is defined as the probability of a transition $\textbf{a} = (a_1, a_2, \dots, a_N) \to \textbf{b} = (b_1, b_2, \dots, b_N)$. For the Moran process, this is as follows:\\
\begin{large}
    \begin{equation}
...
For example, the transition $(0,1) \to (0,2)$. This would be forbidden by the intuition of a standard Moran process, as the new individual is the duplication of another individual in \(\textbf{a}\). 
However, we can see that the standard formula yields \(p_{v,u}\) = 0 because \(\nexists\) \(a_i \in \textbf{a}: a_i = b_{i^*}\). \\
...
We therefore have that the Moran process forms an absorbing Markov chain, and we can investigate the probability of the population being absorbed into each absorbing state.\\

To show how we use this, consider a game with two players, each playing one of two actions. We have a state space
...
Now, by taking the fundamental matrix of \(T\) and performing a right multiplication by \(R\), the matrix with entries which represent transitions from transitive states to absorbing states\cite{Markov_Chain}, we can obtain the absorption matrix for this system.
...
In this matrix, the rows represent our initial transitive state (in our system, row 1 represents \textbf{b} and row 2 represents \textbf{c}), and our columns represent which state the system is absorbed into (with \textbf{a} on the left, and \textbf{d} on the right). 
It is immediately clear from this result that the probability of the system ending up in a particular absorbing state does not rely on the fitness of individuals in the absorbing state, but rather their fitness in the transitive states. Thus, players can remain in a state which could be bettered by a single change, however they will not make such a change if no individual of another type exists in the population.
...
Another method by which we can calculate the probability of one player copying the strategy of another is by using the Fermi function.
Consider a Markov chain where at each time step, one player is chosen with probability \(\frac{1}{N}\) to be the ``focal" player \(a_i\), and they compare their strategy with another player \(a_j\) chosen with probability \(\frac{1}{N-1}\). 
Then, \(a_i\) copies the strategy of \(a_j\) with a probability according to the Fermi function:
\begin{equation}
...
This method has been commonly used in studies of evolutionary games, for example \cite{Maa, Lei, fermi_example}. \\
...
Here we see how the Fermi function can be used to define the transition probability between states in a Markov chain. Importantly, we see that the first case of equation \ref{eqn:imitation_prob} is always in the interval \((0,1)\), as the Fermi function always takes a value on \((0,1)\), and the summation term is at most \(N-1\) values of said function, and thus takes a value \(<N-1\).
...
Now, let us look once more at our state space \(\textbf{a} = (0,0), \textbf{b} = (0,1), \textbf{c} = (1,0), \textbf{d} = (1,1)\). Using this method, we can define a transition matrix (here, this is simplified for ease of reading):
\begin{center}
\begin{equation}
\begin{bmatrix}
1 & 0 & 0 & 0\\\frac{1}{2\left(e^{\frac{- f_{1}{\left(\textbf{b} \right)} + f_{2}{\left(\textbf{b} \right)}}{\beta}} + 1\right)} & 0.5 & 0 & \frac{1}{2\left(e^{\frac{f_{1}{\left(\textbf{b} \right)} - f_{2}{\left(\textbf{b} \right)}}{\beta}} + 1\right)}\\\frac{1}{2\left(e^{\frac{f_{1}{\left(\textbf{c} \right)} - f_{2}{\left(\textbf{c} \right)}}{\beta}} + 1\right)} & 0 & 0.5 & \frac{1}{2\left(2e^{\frac{- f_{1}{\left(\textbf{c} \right)} + f_{2}{\left(\textbf{c} \right)}}{\beta}} + 1\right)}\\0 & 0 & 0 & 1
\end{bmatrix}
\end{equation}
\end{center}
...
\begin{equation}
\begin{bmatrix}
\frac{ e^{\frac{f_{1}{\left(\textbf{b} \right)}}{\beta}}}{\left(e^{\frac{f_{1}{\left(\textbf{b} \right)}}{\beta}} + e^{\frac{f_{2}{\left(\textbf{b} \right)}}{\beta}}\right)} & \frac{1}{\left(e^{\frac{f_{1}{\left(\textbf{b} \right)} - f_{2}{\left(\textbf{b} \right)}}{\beta}} + 1\right)}\\\frac{1}{\left(e^{\frac{f_{1}{\left(\textbf{c} \right)} - f_{2}{\left(\textbf{c} \right)}}{\beta}} + 1\right)} & \frac{ e^{\frac{f_{1}{\left(\textbf{c} \right)}}{\beta}}}{\left(e^{\frac{f_{1}{\left(\textbf{c} \right)}}{\beta}} + e^{\frac{f_{2}{\left(\textbf{c} \right)}}{\beta}}\right)}
\end{bmatrix}
\end{equation}
\end{large}
\end{center}

This function shows that once again, our absorption probabilities rely on the fitness of players in transitive states and not those of players in the absorbing states. %TODO: sentence to lead onto the next section

\subsubsection{Introspection Dynamics}
We will now look at a model of population dynamics in which players do not consider the payoffs of other individuals in the population, but instead at their own possible payoffs. In this dynamic, the following process take place \cite{Introspection Paper 1}
\begin{enumerate}
    \item A random player is chosen with probability \(\frac{1}{N}\) to reconsider their strategy \(a_{i,k}\)
    \item Said player chooses another possible strategy, \(a_{i,l}\), from the set of all possible strategies. 
...
\end{enumerate}
This process is especially strong in a heterogeneous population. 
This is because in some cases using imitation dynamics, a player may copy a strategy due it's fitness when played by another individual in the population, despite the fact that such a strategy actually has a lower payoff for the focal player than their current strategy.
In introspection dynamics, players do not consider the fitness of other individuals, and so cannot be fooled by high fitness strategies with attributes different to that of our focal player.

Couto \cite{Introspection Paper 2} defines the transition matrix for a process according to introspection dynamics as follows:
...
Now, Couto also calculated in \cite{Introspection Paper 1} the explicit strategy distribution for an asymmetric interaction between two players. 
We can adapt these results to define our transition matrix for our example state space \textbf{a,b,c,d}:\\

%TODO get further results on this section giving the 2x2 case. 
...
Therefore, we define a steady state vector \textbf{u} according to the method Couto uses in \cite{Introspection Paper 1}. 
...
Now we consider a population dynamic that utilises this property of introspection dynamics but within the context of an imitative process.
...
T_{\textbf{a,b}} =  \begin{cases}
    \frac{1}{N}\sum_{a_{j} = b_{I(\textbf{a}, \textbf{b})}}\frac{f_j(\textbf{a})}{\sum_{k}f_k(\textbf{a})}\phi(\Delta(f_{I(\textbf{a,b})})) & \text{if \textbf{b}}\in \text{Neb(\textbf{a})}\\
    0 & \text{if \textbf{b}}\notin \text{Neb(\textbf{a}) and \textbf{a}} \neq \textbf{b}\\
...
\rotatebox{90}{\begin{equation}\begin{bmatrix}
1 & 0 & 0 & 0\\\frac{f_{1}{\left(b \right)}}{2 \left(f_{1}{\left(b \right)} + f_{2}{\left(b \right)}\right) \left(e^{\frac{- f_{2}{\left(a \right)} + f_{2}{\left(b \right)}}{\beta}} + 1\right)} & 1 - \frac{f_{2}{\left(b \right)}}{\left(2 f_{1}{\left(b \right)} + 2 f_{2}{\left(b \right)}\right) \left(e^{\frac{f_{1}{\left(b \right)} - f_{1}{\left(d \right)}}{\beta}} + 1\right)} - \frac{f_{1}{\left(b \right)}}{\left(1 + e^{- \frac{f_{2}{\left(a \right)} - f_{2}{\left(b \right)}}{\beta}}\right) \left(2 f_{1}{\left(b \right)} + 2 f_{2}{\left(b \right)}\right)} & 0 & \frac{f_{2}{\left(b \right)}}{2 \left(f_{1}{\left(b \right)} + f_{2}{\left(b \right)}\right) \left(e^{\frac{f_{1}{\left(b \right)} - f_{1}{\left(d \right)}}{\beta}} + 1\right)}\\\frac{f_{2}{\left(c \right)}}{2 \left(f_{1}{\left(c \right)} + f_{2}{\left(c \right)}\right) \left(e^{\frac{- f_{1}{\left(a \right)} + f_{1}{\left(c \right)}}{\beta}} + 1\right)} & 0 & 1 - \frac{f_{1}{\left(c \right)}}{\left(2 f_{1}{\left(c \right)} + 2 f_{2}{\left(c \right)}\right) \left(e^{\frac{f_{2}{\left(c \right)} - f_{2}{\left(d \right)}}{\beta}} + 1\right)} - \frac{f_{2}{\left(c \right)}}{\left(1 + e^{- \frac{f_{1}{\left(a \right)} - f_{1}{\left(c \right)}}{\beta}}\right) \left(2 f_{1}{\left(c \right)} + 2 f_{2}{\left(c \right)}\right)} & \frac{f_{1}{\left(c \right)}}{2 \left(f_{1}{\left(c \right)} + f_{2}{\left(c \right)}\right) \left(e^{\frac{f_{2}{\left(c \right)} - f_{2}{\left(d \right)}}{\beta}} + 1\right)}\\0 & 0 & 0 & 1
...
\begin{equation}
\begin{bmatrix}
\frac{\left(e^{\frac{f_{1}{\left(b \right)} - f_{1}{\left(d \right)}}{\beta}} + 1\right) f_{1}{\left(b \right)}}{\left(e^{\frac{f_{1}{\left(b \right)} - f_{1}{\left(d \right)}}{\beta}} + 1\right) f_{1}{\left(b \right)} + \left(e^{\frac{- f_{2}{\left(a \right)} + f_{2}{\left(b \right)}}{\beta}} + 1\right) f_{2}{\left(b \right)}} & \frac{\left(e^{\frac{- f_{2}{\left(a \right)} + f_{2}{\left(b \right)}}{\beta}} + 1\right) f_{2}{\left(b \right)}}{\left(e^{\frac{f_{1}{\left(b \right)} - f_{1}{\left(d \right)}}{\beta}} + 1\right) f_{1}{\left(b \right)} + \left(e^{\frac{- f_{2}{\left(a \right)} + f_{2}{\left(b \right)}}{\beta}} + 1\right) f_{2}{\left(b \right)}}\\\frac{\left(e^{\frac{f_{2}{\left(c \right)} - f_{2}{\left(d \right)}}{\beta}} + 1\right) f_{2}{\left(c \right)}}{\left(e^{\frac{- f_{1}{\left(a \right)} + f_{1}{\left(c \right)}}{\beta}} + 1\right) f_{1}{\left(c \right)} + \left(e^{\frac{f_{2}{\left(c \right)} - f_{2}{\left(d \right)}}{\beta}} + 1\right) f_{2}{\left(c \right)}} & \frac{\left(e^{\frac{- f_{1}{\left(a \right)} + f_{1}{\left(c \right)}}{\beta}} + 1\right) f_{1}{\left(c \right)}}{\left(e^{\frac{- f_{1}{\left(a \right)} + f_{1}{\left(c \right)}}{\beta}} + 1\right) f_{1}{\left(c \right)} + \left(e^{\frac{f_{2}{\left(c \right)} - f_{2}{\left(d \right)}}{\beta}} + 1\right) f_{2}{\left(c \right)}}
\end{bmatrix}
\end{equation}
\end{large}
\end{center}
...
\begin{tabular}{|c|c|}
\hline
Copy based on target's fitness & Copy based on focal player's fitness\\
\hline
% Figure 1
...
% Stripey diamond (sparser)
\begin{scope}
\clip (7.5,3) -- (8,3.5) -- (8.5,3) -- (8,2.5) -- cycle;
\foreach \x in {7.0,7.1,...,9.0} {
\draw (\x,2.2) -- (\x+1,3.8);
}
\end{scope}
\draw (7.5,3) -- (8,3.5) -- (8.5,3) -- (8,2.5) -- cycle;

% Cross-hatched circle (sparser)
\begin{scope}
\clip (6,1.5) circle (0.5);
\foreach \x in {5.3,5.4,...,6.7} {
\draw (\x,1) -- (\x,2);
}
\foreach \y in {1,1.1,...,2} {
\draw (5.3,\y) -- (6.7,\y);
...
% Dotted triangle in the bottom-right corner
\draw[thick, dotted] (7.5,0.7) -- (8.5,0.7) -- (8,1.5) -- cycle;

\draw[dotted, thick] (8,3) circle (0.8cm);
...
\end{scope}
\draw (13,1.5) -- (12.5,0.7) -- (13.5,0.7) -- cycle;
\node (triC) at (13,0.75) {};
...
\node at (2,4.3) {\textbf{Removal}};
\node at (7,4.3) {\textbf{Duplication}};
\node at (12,4.3) {\textbf{Transformation}};

\node at (7, 5.2) {\textbf{Moran Process}};
...
\begin{scope}[shift={(6,2)}]
    ...
    \draw (0,0.5) -- (-0.5,-0.5) -- (0.5,-0.5) -- cycle;

    % --- less-than symbol ---
    \node at (1,0) {$<$};

    % --- filled black triangle ---
    \fill[black] (2,0.5) -- (1.5,-0.5) -- (2.5,-0.5) -- cycle;
\end{scope}
...
\node at (2,4.3) {\textbf{Selection}};
\node at (7,4.3) {\textbf{Comparison}};
\node at (12,4.3) {\textbf{Transformation}};

\node at (7, 5.2) {\textbf{Introspection Dynamics}};
...
%================ SECOND BOX ==================
\draw (5,0) rectangle (9,4);
% middle comparison: empty triangle < small striped diamond
\begin{scope}[shift={(6,2)}]
    ...
    % --- small striped diamond ---
    \begin{scope}[shift={(2,0)}, scale=0.5] % scale down diamond
        % outline
...
\node at (7, 5.2) {\textbf{Fermi Imitation Dynamics}};


\draw[dotted, thick] (3,3) circle (0.8cm);
\draw[dotted, thick] (3,1) circle (0.9cm);
\end{tikzpicture}
&
% Figure 4
...
\begin{scope}[shift={(6,2)}]
    ...
    % --- filled black triangle ---
    \draw (2,0.5) -- (1.5,-0.5) -- (2.5,-0.5) -- cycle;
    \begin{scope}
            \clip (2,0.5) -- (1.5,-0.5) -- (2.5,-0.5) -- cycle;
            % draw enough stripes from left-bottom to top-right
            \foreach \x in {-1.5,-1.4,...,2} {
                \draw (\x,-1) -- (\x+2,1);
            }
        \end{scope}
\end{scope}
...
\node at (7, 5.2) {\textbf{Introspective Imitation Dynamics}};
...
In this section, we shall see how different population dynamics can be applied to the classical public goods game. 
In this game, individuals choose whether or not to contribute to a public resource pool. 
In the end, the total resource is multiplied by some factor \(r > 1\) and distributed equally between each player. This model often encourages a behaviour known as ``free-riding" \cite{Climate Clubs}, where players refuse to contribute to the pool and simply take the benefit provided by other individuals' contribution. 
...
A common method \cite{Nowak 2006} for this is to apply the exponential function to our payoff function. By using \(e^{\beta \sigma}\) as our fitness function we will have positive values, however this particular approach gives, in the case of the Moran process:
...
The most simple type of public goods game is the homogeneous public goods game.
...
\begin{table}[!h]
    ...
    \begin{tabular}{|c|c|}
    \hline
         Moran Process&Fermi Imitation Dynamics\\
         \hline
         &\\
         $\displaystyle \left[\begin{matrix}\frac{\frac{\alpha r}{2} + \beta}{\alpha r - \alpha + 2 \beta} & \frac{\frac{\alpha r}{2} - \alpha + \beta}{\alpha r - \alpha + 2 \beta}\\\frac{\frac{\alpha r}{2} + \beta}{\alpha r - \alpha + 2 \beta} & \frac{\frac{\alpha r}{2} - \alpha + \beta}{\alpha r - \alpha + 2 \beta}\end{matrix}\right]$
         ...
         &\\
         \hline
         Introspection Dynamics& Introspective Imitation Dynamics\\
         \hline
         &\\
         TODO&
         \small
...
This is clear, as the states \(\textbf{b} = (0,1)\) and \(\textbf{c} = (1,0)\) are equivalent due to there being no difference between players 1 and 2.
...
\bibitem{Introspection Paper 2}
Marta C. Couto, Saptarshi Pal (2023) - Introspection Dynamics in Asymmetric Multiplayer Games 
\bibitem{Evolutionary_Games_Textbook}
Josef Hofbauer, Karl Sigmund (1998, 2003) - Evolutionary Games and Population Dynamics
\bibitem{fermi_example}
Matjaž PercCorresponding Author; Jesús Gómez-Gardeñes; Attila Szolnoki; Luis M. Floría; Yamir Moreno(2013) - Evolutionary dynamics of group interactions on structured populations: a review
\bibitem{Introspection Paper 1}
 M C Couto et al 2022: Introspection dynamics: a simple model of
counterfactual learning in asymmetric games
\bibitem{Fudenberg_Imhof}
Drew Fudenberg and Lorens A. Imhof (2006): Imitation processes with small mutations
\end{thebibliography}
