...
\subsubsection{Transformations of the Fitness Function}

For some population dynamics, we require our fitness function $\pi$ to be
positive. As such, we define $f_i(\textbf{a}) = 1 + \epsilon\pi_i(\textbf{a})$,
for a tunable parameter $\epsilon$. We call $\epsilon$ the selection intensity
of the system, and it determines to what extent a player considers the payoff
when making their decision. A lower value of \(\epsilon\) will result in less
emphasis being placed on the payoff in decision making.

Population dynamics that utilise the Fermi function \cite{PhysRevE.58.69} use a
parameter \(\beta\) called the choice intensity of the function. This parameter
determines the extent to which a player considers the difference between their
current payoff and another payoff when making decisions. This can either be
their own possible payoff upon switching, or the payoff of another individual
in the population, depending on the dynamic.
...
play.\\
Importantly, we also notice that Introspective Imitation Dynamics takes into
