...
\section{The Model}
In this section, similarly to \cite{Introspection Paper 1,Introspection Paper 2}we examine the underlying framework modelling dynamic processes on a fully heterogeneous population. 
...
Our system will be in the form of a Markov chain~\cite{Stewart_Textbook} which moves between states with a probability \(>0\) if and only if $h(\textbf{a, b}) \leq 1$. 
...
\begin{itemize}
  \item The Moran process, first described in \cite{Moran_Original}
  \item Fermi Imitation Dynamics, as famously used in \cite{Szabo_Toke}
  \item Introspection Dynamics, a more modern dynamic defined in \cite{Introspection Paper 1}
\end{itemize}
...
The Moran process \cite{Nowak 2006} is a dynamic corresponding to the following algorithm:
\begin{enumerate}
...
We therefore have that the Moran process forms an absorbing Markov chain \cite{Stewart_Textbook}, and we can investigate the probability of the population being absorbed into each absorbing state.\\
...
Now, by taking the fundamental matrix of \(T\) and performing a right multiplication by \(R\), the matrix with entries which represent transitions from transitive states to absorbing states\cite{Stewart_Textbook}, we can obtain the absorption matrix for this system.
...
\begin{itemize}
  \item Two players, \(i\) and \(j\) are selected at random
...
example \cite{Maa, Lei, fermi_example,Szabo_Toke}.
...
In introspection dynamics, the following process take place \cite{Introspection Paper 1}
\begin{enumerate}
...
\end{enumerate}
\cite{Introspection Paper 1} considers this process for
the two player case, and it is expanded to multiple players in
\cite{Introspection Paper 2}.
...
The transition matrix for the introspection dynamics process~\cite{Introspection Paper 2} is given by:
...
to the classical public goods game. \cite{Calculus_Of_Selfishness, Evolutionary_Games_Textbook}
...
The most common type of public goods game is the homogeneous public goods game.\cite{Calculus_Of_Selfishness}
...
May 2023 establishing a carbon border adjustment mechanism (Text with EEA
relevance)
...
Drew Fudenberg and Lorens A. Imhof (2006) - Imitation processes with small
mutations
\bibitem{Moran_Original}
P.A.P Moran (1958): Random processes in genetics
\bibitem{Szabo_Toke}
György Szabó and Csaba Tőke (1998) - Evolutionary prisoner’s dilemma game on a
square lattice
\bibitem{Calculus_Of_Selfishness}
Karl Sigmund (2010) - The Calculus of Selfishness
\bibitem{Tragedy_Of_Commons}
