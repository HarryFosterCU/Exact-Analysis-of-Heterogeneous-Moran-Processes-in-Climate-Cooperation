...
\subsection{The Linear and Log-Linear Cases}
...
Now importantly, this case has that each player \(j\) contributes some
\(C_{j-1} + \frac{2M}{N(N+1_)}\), and so the increase in contribution as you
move from one player to the next is constant. We also introduce a case where
this increase is logarithmic, namely so that \(\sum_{j=1}^{N} \ln(j\lambda) =
M\). In this case we define \(\lambda = (\frac{e^M}{N!})^{1/N}\). This case
keeps the difference between contributions from becoming extremely large if we
increase \(M\) while keeping \(N\) constant.

\subsection{The Binomial Case}

In this case, we split the population into two subgroups. These groups are
called "poor" and "wealthy", and their contributions are defined as \(C_{low}\)
and \(C_{high}\) respectively. In this case, we can define a system by the
following parameters:

\begin{itemize}
    \item N - the number of players
    \item M - the maximum population contribution
    \item \(C_{high}\) - the contribution of the wealthy players
    \item n - the number of poor players
\end{itemize}

In this case we have that the following holds:

\begin{equation} \label{eqn:binomial}
n C_{low} + (N-n)C_{high} = NC_{high} - nd = M, \text{where d is the
    difference between low and high contributions.}
\end{equation}

We can therefore, for a given \(M, N, and n\), acquire the bound:

\begin{equation}
    \frac{M}{N} < C_{high} < \frac{M}{N-n}
\end{equation}

As \(C_{high}\) must be greater than \(C_{low}\), and we have \(N-n\) players
contributing \(C_{high}\), so it must be bounded from above.

We also trivially obtain \(C_{low} = \frac{M - (N-n)C_{high}}{n}\), completing
our system. We do not need to define a \(\lambda\) in this system, as we have
\ref{eqn:binomial} that aptly provides us with a system of contributions.\\

Now, as the following cases are defined according to a random distribution, let
us take a detour and look into the distribution that we shall use.
